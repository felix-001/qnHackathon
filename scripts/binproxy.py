...
BIN_MANIFESTS = os.getenv('BIN_MANIFESTS', os.path.join(os.path.dirname(__file__), 'bin-manifests.json'))
    ...
    if lock_file.exists():
        try:
            ...
            if elapsed < LOCK_TIMEOUT:
                log(f"Lock exists for {bin_name}-{bin_hash} (held for {elapsed}s), skipping")
                return False
            else:
                log(f"Stale lock detected for {bin_name}-{bin_hash} (held for {elapsed}s), removing")
...
def report_completion(bin_name: str, bin_hash: str, status: str):
    ...
    try:
        ...
        log(f"Reported completion for {bin_name}-{bin_hash}: {status} (took {elapsed}s)")
    except Exception as e:
...
def post_update_status(bin_name: str, new_sha256: str) -> bool:
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            error(f"Failed to post update status for {bin_name}: {response.status_code}")
            return False
    except Exception as e:
...
def download_binary(bin_name: str, temp_file: str) -> bool:
    ...
    if available < required_space:
        error(f"Insufficient disk space in /tmp (available: {available}KB, required: {required_space}KB)")
        return False
...
def change_version(bin_name: str, target_sha256: str, operation: str = "upgrade") -> bool:
    ...
    if operation == "rollback":
        ...
        if not os.path.exists(source_file):
            error(f"Rollback failed: archived binary not found for SHA256: {target_sha256}")
            return False
        log(f"Using archived binary for rollback: {source_file}")
    else:
        kill_old_downloads(bin_name)
        
        with tempfile.NamedTemporaryFile(mode='wb', delete=False, prefix=f"{bin_name}.tmp.", dir='/tmp') as tmp:
        ...
        if downloaded_sha256 != target_sha256:
            error(f"SHA256 mismatch for downloaded {bin_name} (expected: {target_sha256}, got: {downloaded_sha256})")
...
def record_version_change(bin_name: str, from_sha256: str, to_sha256: str, operation: str, result: str):
    ...
    try:
        ...
        log(f"Recorded version change: {bin_name} ({operation}: {from_sha256} -> {to_sha256}, result: {result})")
    except Exception as e:
